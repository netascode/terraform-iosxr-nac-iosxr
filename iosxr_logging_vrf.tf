locals {
  device_logging_vrfs = flatten([
    for device in local.devices : [
      for logging_vrf in try(local.device_config[device.name].logging_vrf, []) : {
        device_name = device.name
        vrf_name    = try(logging_vrf.vrf_name, null)
        key         = "${device.name}-${logging_vrf.vrf_name}"

        ipv4_hosts = try(length(logging_vrf.host_ipv4_addresses) == 0, true) ? null : [for host in logging_vrf.host_ipv4_addresses : {
        ...
        ipv6_hosts = try(length(logging_vrf.host_ipv6_addresses) == 0, true) ? null : [for host in logging_vrf.host_ipv6_addresses : {
...
resource "iosxr_logging_vrf" "logging_vrf" {
  for_each = { for logging_vrf in local.device_logging_vrfs : logging_vrf.key => logging_vrf }
  device   = each.value.device_name
  vrf_name = each.value.vrf_name
  host_ipv4_addresses          = each.value.ipv4_hosts
  host_ipv6_addresses          = each.value.ipv6_hosts
}
