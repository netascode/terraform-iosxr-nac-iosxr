    ...
    for device in local.devices : [
      for route in try(local.device_config[device.name].routing.static_routes.address_family.ipv6_unicast.prefixes, []) : {
        ...
        sr_policies = try(length(route.sr_policies) == 0, true) ? null : [for sr_policy in route.sr_policies : {
          ...
          distance_metric = try(sr_policy.distance_metric, local.defaults.iosxr.devices.configuration.routing.static_routes.address_family.ipv6_unicast.prefixes.sr_policies.distance_metric, null)
    ...
    for device in local.devices : [
      for route in try(local.device_config[device.name].routing.static_routes.address_family.ipv6_multicast.prefixes, []) : {
        ...
        sr_policies = try(length(route.sr_policies) == 0, true) ? null : [for sr_policy in route.sr_policies : {
          ...
          distance_metric = try(sr_policy.distance_metric, local.defaults.iosxr.devices.configuration.routing.static_routes.address_family.ipv6_multicast.prefixes.sr_policies.distance_metric, null)
