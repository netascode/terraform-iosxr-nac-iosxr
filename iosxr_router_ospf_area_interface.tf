      ...
      for ospf_process in try(local.device_config[device.name].routing.ospf_processes, []) : [
        for area in try(ospf_process.areas, []) : [
          for interface in try(area.interfaces, []) : {
            ...
            fast_reroute_per_prefix_ti_lfa                     = try(interface.fast_reroute_per_prefix_ti_lfa, local.defaults.iosxr.devices.configuration.routing.ospf_processes.areas.interfaces.fast_reroute_per_prefix_ti_lfa, null)
            fast_reroute_per_prefix_tiebreaker_node_protecting = try(interface.fast_reroute_per_prefix_tiebreaker_node_protecting, local.defaults.iosxr.devices.configuration.routing.ospf_processes.areas.interfaces.fast_reroute_per_prefix_tiebreaker_node_protecting, null)
            fast_reroute_per_prefix_tiebreaker_srlg_disjoint   = try(interface.fast_reroute_per_prefix_tiebreaker_srlg_disjoint, local.defaults.iosxr.devices.configuration.routing.ospf_processes.areas.interfaces.fast_reroute_per_prefix_tiebreaker_srlg_disjoint, null)
