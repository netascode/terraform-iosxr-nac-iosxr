resource "iosxr_domain" "domain" {
  for_each = { for device in local.devices : device.name => device if try(local.device_config[device.name].domain, null) != null }
  device   = each.value.name

  name                    = try(local.device_config[each.value.name].domain.name, local.defaults.iosxr.configuration.domain.name, null)
  domains                 = try(length(local.device_config[each.value.name].domain.domains) == 0, true) ? null : try(local.device_config[each.value.name].domain.domains, local.defaults.iosxr.configuration.domain.domains, null)
  ...
  name_servers            = try(length(local.device_config[each.value.name].domain.name_servers) == 0, true) ? null : try(local.device_config[each.value.name].domain.name_servers, local.defaults.iosxr.configuration.domain.name_servers, null)
