  ...
  device_evpn_groups = flatten([
    for device in local.devices : [
      for evpn_group in try(local.device_config[device.name].evpn_group, []) : {
        ...
        key         = "${device.name}-evpn-group-${evpn_group.group_id}"
    ...
    if try(local.device_config[device.name].evpn_group, null) != null || try(local.defaults.iosxr.configuration.evpn_group, null) != null
  ])
}

resource "iosxr_evpn_group" "evpn_group" {
  for_each = { for evpn_group in local.device_evpn_groups : evpn_group.key => evpn_group }
